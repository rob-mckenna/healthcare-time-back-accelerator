...
| REQ-SCOPE-002 | One Microsoft Foundry Shift Closeout Agent | §3 | Neo | `contracts/schemas/shift-closeout-agent-input.schema.json`, `contracts/schemas/shift-closeout-agent-output.schema.json` | `npm run validate:contracts` | Baseline accepted; exactly one Prompt Agent provisioned and verified (no tools) 2026-08-13, `docs/evidence/2026-08-13-foundry-agent-provisioning.md`; live behavioral verification blocked, see BLOCKER-003 |
...
| REQ-AGT-004 | Every generated section carries source references that resolve to approved synthetic input | §3, §5.6 | Neo | `contracts/schemas/source-reference.schema.json` | `rule source-reference-resolution`; `npm run demo` M4; `npm run evaluate:grounding` | Verified locally (example set); live verification blocked on BLOCKER-003 / RISK-019 — see `docs/agent/grounding-evaluation.md` |
